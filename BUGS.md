...
- **BUG-005** `[dylib/tap]` `status:in-progress` — `tap.tab` fails with SwiftUI `TabView`: "No tab bar found in view hierarchy". `findTabBarButtons()` only searches for `UITabBar` and class names containing "TabBar", but SwiftUI `TabView` renders tab buttons as accessibility elements, not UIKit views. Workaround: use `tap text:"TabName"`. *(found: 2026-03-22, GH #8)*
