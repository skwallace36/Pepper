...
def _is_session_live(session: dict) -> bool:
    ...
    if claimed_str:
        try:
            ...
            age = (datetime.now(datetime.UTC) - claimed_time).total_seconds()
    ...
    if _is_pid_alive(pid):
        ...
        if heartbeat_str:
            try:
                ...
                age = (datetime.now(datetime.UTC) - heartbeat_time).total_seconds()
...
def heartbeat(udid: str, bundle_id: str = "", port: int = 0):
    ...
    if not session or session.get("pid") != os.getpid():
        return
    session["heartbeat"] = datetime.now(datetime.UTC).isoformat()
    if bundle_id:
