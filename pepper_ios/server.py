...
from pepper_ios import sessions as pepper_sessions
from pepper_ios.mcp_build import (
...
from pepper_ios.mcp_crash import fetch_crash_info
from pepper_ios.mcp_tools_debug import register_debug_tools
from pepper_ios.mcp_tools_nav import register_nav_tools
from pepper_ios.mcp_tools_record import register_record_tools
from pepper_ios.mcp_tools_renders import register_renders_tools
from pepper_ios.mcp_tools_sim import register_sim_tools
from pepper_ios.mcp_tools_state import register_state_tools
from pepper_ios.mcp_tools_system import register_system_tools
from pepper_ios.websocket_client import CrashError, make_command
from pepper_ios.websocket_client import send_command as ws_send_command
...
# ---------------------------------------------------------------------------
# Config from .env (shared via pepper_ios.common)
# ---------------------------------------------------------------------------

from pepper_ios.common import (
    PEPPER_DIR, PORT_DIR, load_env, get_config,
    discover_port, discover_simulator, discover_instance, list_simulators,
    list_instances, try_parse_json, require_parse_json,
)
...
async def send_command(port: int, cmd: str, params: Optional[dict] = None,
                       timeout: float = 10, host: str = "localhost") -> dict:
    ...
    addr = f"{host}:{port}"
    try:
        return await ws_send_command(host, port, msg, timeout=timeout)
...
async def resolve_and_send(simulator: Optional[str], cmd: str, params: Optional[dict] = None, timeout: float = 10) -> str:
    """Resolve instance and send command. Returns JSON string. Auto-appends crash info on APP CRASHED.
...
async def act_and_look(simulator: Optional[str], cmd: str, params: Optional[dict] = None, timeout: float = 10) -> str:
    ...
    from pepper_ios.format import format_look
    ...
    telemetry_task = asyncio.create_task(
        _gather_telemetry(port, pre_counts, bound_fn)
    )
...
register_renders_tools(mcp, resolve_and_send)
register_system_tools(mcp, resolve_and_send, act_and_look, _resolve_simulator)
register_record_tools(mcp)
register_sim_tools(mcp, resolve_and_send, _resolve_simulator)
...
async def deploy(
    ...
    skip_privacy: bool = Field(default=False, description="Skip auto-granting privacy permissions (photos, camera, microphone, contacts, calendar, location)"),
) -> str:
    ...
    By default, grants common privacy permissions before launch (opt out with skip_privacy=true)."""
    try:
        sim = _resolve_simulator(simulator)
    except RuntimeError as e:
        return str(e)
    return await _deploy_app(sim, send_fn=send_command, bundle_id=bundle_id, dylib_path=dylib_path, skip_privacy=skip_privacy)
...
async def webview(
    ...
    script: Optional[str] = Field(default=None, description="JavaScript to execute (for evaluate)"),
    selector: Optional[str] = Field(default=None, description="CSS selector for DOM query (default: '*', for dom)"),
    index: Optional[int] = Field(default=None, description="WKWebView index when multiple are present (default: 0)"),
    limit: Optional[int] = Field(default=None, description="Max DOM elements to return (default: 50, for dom)"),
) -> str:
