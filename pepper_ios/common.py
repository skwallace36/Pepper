...
def _find_dylib() -> str:
    """Find Pepper.framework dylib -- check env, package data, then build dir."""
    env = load_env()
    if env.get("APP_DYLIB_PATH"):
        return env["APP_DYLIB_PATH"]
    # Check package data (pip install mode)
    pkg_dylib = os.path.join(os.path.dirname(__file__), "data", "Pepper.framework", "Pepper")
    if os.path.exists(pkg_dylib):
        return pkg_dylib
    # Fall back to build dir (development mode)
