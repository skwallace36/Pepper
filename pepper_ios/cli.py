...
from pepper_ios.common import (
    DEFAULT_HOST, PORT_DIR, load_env,
    discover_port as _discover_port, discover_instance,
    register_device as _register_device, unregister_device as _unregister_device,
    list_instances as _list_instances,
)
from pepper_ios import format as pepper_format
...
""",
    ...
    parser.add_argument("--host", default=None, help=f"Server host (default: auto-detect, or {DEFAULT_HOST} for simulators)")
    ...
    parser.add_argument("--device", "-d", default=None, help="Device UDID to connect to (reads host:port from /tmp/pepper-devices/<UDID>.device)")
    ...
    p.add_argument("--format", choices=["junit", "json"], default="junit", dest="report_format", help="Output format (default: junit)")
...
async def cmd_wait_for_server(args):
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            if not args.raw_json:
                print(red(f"Timeout: server not reachable after {args.wait_timeout}s ({attempt} attempts)"), file=sys.stderr)
            else:
...
async def cmd_test_report(args):
        ...
        try:
            ...
            if resp_status == "ok":
                ...
                if assertion_failure:
                    ...
                    if not args.raw_json:
                        print(red(f" FAIL") + dim(f" ({elapsed:.2f}s) {assertion_failure}"), file=sys.stderr)
                    if not args.continue_on_error:
                        break
                else:
                    results.append({
                ...
                if not args.raw_json:
                    print(red(f" FAIL") + dim(f" ({elapsed:.2f}s) {err_msg}"), file=sys.stderr)
                if not args.continue_on_error:
                    break
        except ConnectionRefusedError:
            elapsed = time.monotonic() - t0
            ...
            if not args.raw_json:
                print(red(f" CRASH") + dim(f" ({elapsed:.2f}s)"), file=sys.stderr)
            break
    ...
    if not args.raw_json:
        ...
        print(color(f"\n{total} tests: {num_pass} passed, {num_fail} failed, {num_error} errors ({suite_time:.2f}s)"), file=sys.stderr)
