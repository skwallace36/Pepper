...
- **TASK-063** `[P3]` `status:in-progress` — Extract `pepper_websocket.py`: shared `send_command()` with event filtering, crash detection, ID matching. Deduplicate pepper-mcp and pepper-ctl WebSocket logic. Merge pepper-ctl's redundant `send_command()` / `send_and_recv_multi()`
