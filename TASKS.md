...
- **TASK-115** `[P3]` `status:in-progress` — Create `IOSDialogDetection` wrapping `PepperDialogInterceptor.shared` + `IOSNavigationBridge` wrapping `PepperNavBridge` + `IOSViewIntrospection` wrapping existing layer/heap code. *(blocked by TASK-110)*
