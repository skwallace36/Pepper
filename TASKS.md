...
- **TASK-061** `[P3]` `status:in-progress` — Extract port discovery to `pepper_common.py`: `discover_port()`, `discover_simulator()`, `list_simulators()`. Consolidate 4 reimplementations (pepper-mcp, pepper-ctl, pepper-stream, test-client.py) into one with liveness checks
