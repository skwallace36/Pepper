...
- **TASK-080** `[P3]` `status:in-progress` — Add `pepper-ctl wait-for-server` health check command (poll WebSocket until connected or timeout)
...
- **TASK-115** `[P3]` `status:pr-open` — Create `IOSDialogDetection` wrapping `PepperDialogInterceptor.shared` + `IOSNavigationBridge` wrapping `PepperNavBridge` + `IOSViewIntrospection` wrapping existing layer/heap code. *(blocked by TASK-110)*
